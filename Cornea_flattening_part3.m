...
Ext=name(end-3:end);
mkdir(Path,'Flat_EPI'); % Create a subfolder in the directeory
mkdir(Path, 'Average_Flat_EPI_ENFACE');
Path_save=[Path,'Flat_EPI\'];
Path_save_avg=[Path, 'Average_Flat_EPI_ENFACE\'];
...
% %% top view using average stack to reduce artifact
% parfor middle_frame = max(3, min(y1, y2)):max(y1, y2)
%     top_view = zeros(1000, column);
%     for i = 1:1000
%        name_2 = num2str(i); 
%        frame = dicomread([address_a, strcat(name_1, name_2, name_3)]);
%        %frame = imgaussfilt(frame, 1);
%        slide_1 = double(frame(middle_frame, :));
%        slide_2 = double(frame(middle_frame + 1, :));
%        slide_3 = double(frame(middle_frame + 2, :));
%        slide_4 = double(frame(middle_frame + 3, :));
%        %slide_5 = double(frame(middle_frame + 2, :));%have to set it to double, otherwise smooth function won't work
%        top_view(i, :) = (slide_1 + slide_2 + slide_3 + slide_4) / 4;
%     end
% 
%     if strcmp(Cl,'uint8')==1
...
% %% top view test
% 
% middle_frame = peak_point + y1 - 1;
% top_view = zeros(1000, column);
% for i = 1:1000
%    name_2 = num2str(i); 
%    frame = dicomread([address_a, strcat(name_1, name_2, name_3)]);
%    %frame = stdfilt(frame);
%    %frame = imgaussfilt(frame, 0.7);
%    %slide_1 = double(frame(middle_frame - 2, :));
%    slide_2 = double(frame(middle_frame - 1, :));
%    slide_3 = double(frame(middle_frame, :));
%    slide_4 = double(frame(middle_frame + 1, :));
%    %slide_5 = double(frame(middle_frame + 2, :));%have to set it to double, otherwise smooth function won't work
%    %top_view(i, :) = imgaussfilt((slide_1 + slide_2 + slide_3 + slide_4 + slide_5) / 5, 0.4);
%    top_view(i, :) = (slide_2 + slide_3 + slide_4);
%    %top_view(i, :) = slide_3;
% end
% 
% %top_view = imgaussfilt(top_view, [1 0.5]); guassian filter doesn't remove
% %new_top_view = stdfilt(top_view, true(17));
% %the artifacts
