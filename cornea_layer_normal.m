function [miceye1, lower2, midd2, upper2] = cornea_layer_normal(path, num1, upper_depth, middle_depth, lower_depth)
...
%     colum_smooth = smooth(colum_avg, 5);
    ...
    while true
        if start < eyer - 50
           if max(colum_smooth((start - 50) : (start - 10))) > 0 && upper_depth * max(colum_smooth((start - 50) : (start - 10))) < colum_smooth(start)
...
m = 21;
while m <= length(upper1)
    if 2 * (max(upper1(2, (m - 20):(m - 5))) - min(upper1(2, (m - 20):(m - 5)))) < abs(upper1(2, m) - upper1(2, m - 1)) ...
            && max(upper1(2, (m - 20):(m - 5))) - min(upper1(2, (m - 20):(m - 5))) > 0
        upper1(:, m) = [];
    elseif m == length(upper1)
        break
    else
        m = m + 1;
    end
...
k = length(upper1) - 21;
while k >= 1
    if k + 21 <= length(upper1) && 2 * (max(upper1(2, (k + 5):(k + 20))) - min(upper1(2, (k + 5):(k + 20)))) < abs(upper1(2, k) - upper1(2, k + 1)) ...
            && max(upper1(2, (k + 5):(k + 20))) - min(upper1(2, (k + 5):(k + 20))) > 0
        upper1(:, k) = [];
    elseif k == 1
        break
    else
...
%     colum_avg = (miccolum_avg(:, index) + miceye2(:, index)) / 2;
    ...
    while bound + 80 < eyer 
        if start <  bound 
            if middle_depth * max(colucom_avg((start - 20):(start - 10))) < colum_avg(start) && mean(colum_avg((start + 10):(start + 20))) > 0.02            
...
% cord = 100;
% colum_one = (miccolum_avg(:, cord) + miceye2(:, cord) + miceye3(:, cord) + miceye4(:, cord)) / 4;
