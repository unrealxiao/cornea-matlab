 %% test cornea_layer
path = '\\iu-opt-research\\TankamLab\\OCM mice image\\4Th Aug2021\\Mice_Female_7_KR_TC_Smad4FF_PBS_1V\\';
% num1 = 1;
...
%% lower_3D aquire
parfor i = 10:997
    [~, lower2, midd2, upper2] = cornea_layer_notnormal(path, i, 2, 2, 1.5);
...
%% plot the depth
 ...
 imagesc(epi_stroma_ko * 0.7);title('epi');
 caxis([0 110]);
figure;
imagesc(stroma_endo_ko * 0.7);title('stroma');
 caxis([0 110]);
figure;
imagesc(epi_endo_ko * 0.7);title('whole');
 caxis([0 110]);
%%
real_epi_stroma = epi_stroma_ko * 0.7;
real_epi_endo = epi_endo_ko * 0.7;
real_stroma_endo = stroma_endo_ko * 0.7;
%epi
